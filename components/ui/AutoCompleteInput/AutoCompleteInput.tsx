...
const AutoCompleteInput: React.FC<SelectInputProps> = ({ options, onChange, value, disabled }) => {
    ...
    const autoCompleteAPI = async (value: string) => {
        try {
            ...
            setItems(response?.msg?.predictions?.map((item: any) => ({ "id": item.place_id, "name": item.description })))
        } catch (error) {
    ...
    const handleSelectedItem = (event: any, index = undefined) => {
        ...
        if (index !== undefined) {
            ...
            switch (event.key) {
                ...
                case 'ArrowUp':
                    event.preventDefault()
                    itemIndex = highlightedItem > -1 ? highlightedItem - 1 : items.length - 1
                    setHighlightedItem(itemIndex);
                    break
                case 'ArrowDown':
                    event.preventDefault()
                    itemIndex = highlightedItem < items.length - 1 ? highlightedItem + 1 : -1
                    setHighlightedItem(itemIndex);
                    break
                default:
                    break
            ...
            <div style={{ width: "380px" }}>
                <div className={styles.input_container} >
                    <div style={{ position: 'relative', width: "380px" }} >
                        <div style={{ display: 'flex', alignItems: "center", position: "absolute", height: "100%", marginLeft: "10px" }}>
                        ...
                        <input
                            ...
                            disabled={disabled}
                        />
                        <div style={{ position: "absolute", width: "100%", border: searchString ? '1px solid #ccc' : "none" }}>
                            {searchString && items?.map((val, index) => {
                                return <div
                                    style={{ padding: "4px 10px 4px 10px", cursor: "pointer", background: highlightedItem === index ? "#fbf0e8" : "" }}
                                    onClick={() => {
                                        setSearchString('');
