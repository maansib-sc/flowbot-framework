...
const TextInput: React.FC<TextInputProps> = ({ label, value, width, disabled, containerwidth, onChange }) => {
  const inputStyle = {
  ...
  return (
    <div className={styles.input_container} style={containerStyle}>
      <span>{label}</span>
      <div className={styles.input_container} style={containerStyle}>
        <input style={inputStyle} className={styles.password_input} value={value} onChange={onChange} disabled={disabled} />
      </div>
    </div>
  );
};
