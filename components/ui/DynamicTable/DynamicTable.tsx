...
const DynamicTable: React.FC<TableProps<string[]>> = ({
  data,
  onChange,
  options,
  total,
  ...
  const handleContentChange = (event:any, index: number, childIndex: number) => {
  ...
  return (
    <>
      <table className={styles.table + ' table-auto'}>
        <thead>
          <tr>
            {useData?.name?.map((item: any, index) => (
              <th
                key={index}
                ...
                className={` ${
                  index === data.name.length - 1
                    ? 'text-end'
                    : index !== data.name.length - 1 && index !== 0
                    ? 'text-center'
                    : 'text-start'
                }`}
              >
        ...
        <tbody>
          {useData?.data?.map((item, index) => (
            <tr
              key={index}
              className={editableRow === index ? styles.selectedRow : ''}
              ref={containerRef}
            >
              {data?.name?.map((value, newindex) => [
                <td
                  key={newindex}
                  className={` ${
                    newindex === data.name.length - 1
                      ? 'text-end'
                      : newindex !== data.name.length - 1 && newindex !== 0
                      ? 'text-center'
                      : 'text-start'
                  }`}
                >
                  {editableRow === index ? (
                    <span>
                      <input
                        value={item[newindex]}
                        onChange={(event) =>
                          handleContentChange(event, index, newindex)
                        }
                      />
                    </span>
                  ) : (
              ...
              {options?.editable && (
                <td className="text-end">
                  ...
                  >
                    ...
                    >
                      <path
                        d="M10 14.6719C10.6903 14.6719 11.25 15.2315 11.25 15.9219C11.25 16.6122 10.6903 17.1719 10 17.1719C9.30967 17.1719 8.75 16.6122 8.75 15.9219C8.75 15.2315 9.30967 14.6719 10 14.6719ZM10 9.67188C10.6903 9.67188 11.25 10.2315 11.25 10.9219C11.25 11.6122 10.6903 12.1719 10 12.1719C9.30967 12.1719 8.75 11.6122 8.75 10.9219C8.75 10.2315 9.30967 9.67188 10 9.67188ZM10 4.67188C10.6903 4.67188 11.25 5.23152 11.25 5.92188C11.25 6.61223 10.6903 7.17188 10 7.17188C9.30967 7.17188 8.75 6.61223 8.75 5.92188C8.75 5.23152 9.30967 4.67188 10 4.67188Z"
                        fill="#727A8B"
                      />
                    </svg>
      ...
      {total && (
        <div className={styles.tableFooter}>
          ...
          <span className={styles.price} style={{ marginRight: '14px' }}>
            ${total.value}
          </span>
        </div>
      )}
      {options?.bottomAddButton && !disabled && (
        <div
          className="flex gap-2 align-middle mt-4 cursor-pointer w-fit"
          onClick={addRow}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      {showconfirmButton && showButton && (
        <div className="mt-4">
          <Button
            onClick={() => {
              onChange(JSON.stringify(useData));
              setShowButton(false);
            }}
          >
            Confirm
